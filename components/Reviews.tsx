      ...
      <div className="max-w-7xl mx-auto px-4 relative z-20">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
               <div className="bg-white border border-gray-200 p-2 rounded-lg shadow-sm">
                 ...
                 <img src="https://upload.wikimedia.org/wikipedia/commons/a/ad/Yelp_Logo.svg" alt="Yelp" className="h-8 w-auto" />
               </div>
               <span className="px-4 py-1.5 bg-green-100 border border-green-200 text-green-800 text-sm font-bold rounded-full">Verified Business</span>
            </div>
            
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-navy-900 mb-2">Bay Area Favorites</h2>
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700 font-bold text-lg">5.0 Average Rating</span>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
             <div className="flex bg-gray-100 border border-gray-200 p-1 rounded-xl overflow-hidden">
              {filters.map((item) => (
                <button
                  key={item.key}
                  onClick={() => setFilter(item.key as any)}
                  className={`px-5 py-3 rounded-lg text-base font-bold transition-all duration-300 ${
                    filter === item.key 
            ...
            <a 
              ...
              className="bg-brand-red hover:bg-red-700 text-white px-8 py-3 rounded-xl font-bold text-base transition-all shadow-lg shadow-brand-red/20 hover:shadow-brand-red/40 flex items-center justify-center gap-2"
            >
              Check our Yelp
            </a>
          </div>
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
          {filteredReviews.map((review) => (
            <div key={review.id} className="break-inside-avoid bg-white p-8 rounded-2xl shadow-lg border border-gray-100 transition-all duration-500 hover:shadow-2xl hover:border-gold-400/30 hover:-translate-y-1 relative group backdrop-blur-sm">
              
              {/* Animated Quote Icon */}
              <Quote className="absolute top-6 right-6 w-10 h-10 text-gray-200 fill-gray-200 group-hover:text-gold-200 group-hover:fill-gold-200 transition-all duration-500 group-hover:scale-110 group-hover:-rotate-12" />
              ...
              <p className="text-gray-800 leading-relaxed mb-6 font-medium text-xl">"{review.text}"</p>
              
              <div className="flex items-center gap-3 pt-6 border-t border-gray-100">
                 <div className="w-12 h-12 rounded-full bg-navy-900 flex items-center justify-center text-white font-bold text-base shadow-sm">
