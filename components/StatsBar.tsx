...
const Counter: React.FC<{ end: number; duration: number; suffix: string }> = ({ end, duration, suffix }) => {
  ...
  return (
    <span ref={ref} className="font-bold text-4xl md:text-5xl text-navy-900 font-serif tracking-tight">
  ...
  return (
    <section className="relative z-30 -mt-20 px-4">
      <div className="max-w-7xl mx-auto">
        {/* White Glass Card */}
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/40 p-8 md:p-12 relative overflow-hidden transition-transform hover:-translate-y-1 duration-500 group">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center divide-x-0 md:divide-x divide-gray-200 relative z-20">
            {statsData.map((stat) => (
              <div key={stat.id} className="flex flex-col items-center justify-center p-2 transition-colors hover:bg-black/5 rounded-lg">
                ...
                <span className="text-gray-700 uppercase tracking-widest text-sm font-bold mt-3 font-sans group-hover:text-gold-700 transition-colors">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
